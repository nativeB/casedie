...
class Pacman {
  constructor(options) {
    ...
    if (this.options.renameFiles) {
      this.renameFiles(null, this.buildDirectory(
          this.options.path, null, 0), null, null);
    }
  }
    ...
    try {
      ...
      if (this.options.deep) {
        for (let i = 0; i < dir.length; i++) {
          const stat = this.getStats(path, dir[i]);
  ...
  renameFiles(casetouse, files, dirpath, cb) {
    // if(!files)
    const filesList=files||this.buildDirectory(
        this.options.path||dirpath, null, 0);
    let newFileName = '';
    let toPath = '';
    let fromPath; let fileName; let extention;
    filesList.forEach((file) =>{
      ...
      const casing = casetouse||this.options.case;
      // console.log(casing)
      switch (casing) {
        case 'camel':
  ...
  replaceInFiles(fileList, cb) {
    ...
    replace({
      ...
      for (let i = 0; i <results.length; i++) {
        fileList[i]={...fileList[i], ...results[i]};
      }
...
// let pac= new Pacman({
// path:'./components',deep:true,renameFiles:true,
// case:'camel',replaceInFiles:true,
// cb:(files)=>{
// console.log('this renamed',files)}
// })
